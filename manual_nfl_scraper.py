"""
Manual NFL.com Injury Scraper
Based on the actual games and injury data visible on the NFL.com injury report page
"""
...
class ManualNFLInjuryScraper:
    """
    Manual scraper that extracts injury data based on the actual NFL.com structure
    """
    ...
    def scrape_all_injuries(self) -> Dict:
        ...
        try:
            ...
            # Process each table and try to identify which team it belongs to
            for i, table in enumerate(all_tables):
                team_name, injuries = self._process_table(table, i)
    ...
    def _process_table(self, table, table_index: int) -> tuple:
        """Process a single table to extract team and injuries"""
        try:
            # Extract injuries first
            injuries = self._extract_injuries_from_table(table)
            
            if not injuries:
                return None, []
            
            # Try to identify the team for this table
            team_name = self._identify_team_for_table(table, table_index)
    ...
    def _identify_team_for_table(self, table, table_index: int) -> Optional[str]:
        """Identify which team a table belongs to"""
        try:
            # Get all text from the table and its context
            context_text = ""
            ...
            # If we found exactly one team, use it
            if len(found_teams) == 1:
                return found_teams[0]
            
            # If we found multiple teams, try to determine which one this table belongs to
            ...
            # If we still can't identify, return the first found team
            if found_teams:
                return found_teams[0]
            ...
            logger.error(f"Error identifying team for table: {e}")
            return None
...
if __name__ == "__main__":
    # Test the manual scraper
    scraper = ManualNFLInjuryScraper()
